...
def chat_ollama(current_model, prompt, image, timeout_seconds=40):
    with multiprocessing.Pool(processes=1) as pool:
        async_result = pool.apply_async(call_ollama_chat, (current_model, prompt, image))
        try:
            print(f"Prompting {current_model} with timeout {timeout_seconds}s")
